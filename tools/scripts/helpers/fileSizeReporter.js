...
function getDifferenceLabel(currentSize, previousSize) {
  ...
  if (difference >= FIFTY_KILOBYTES) {
    return console.log(chalk.red(`+${fileSize}`));
  } else if (difference < FIFTY_KILOBYTES && difference > 0) {
    return console.log(chalk.yellow(`+${fileSize}`));
  } else if (difference < 0) {
    return console.log(chalk.green(fileSize));
  }
  return '';
}
    ...
    recursive(buildFolder, (err, fileNames) => {
      ...
      if (!err && fileNames) {
        sizes = fileNames
          ...
          .reduce((memoParams, fileName) => {
            const memo = memoParams;
